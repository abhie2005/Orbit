...
**Last updated:** 2026-09-12 — landing hero is type only (Claude Code)
...
**Latest visual change:** the landing hero is **type only**. No graphic of any
kind — no galaxy scene, no particle sphere, no constellation diagram. Abhi ran
through all three and cut them all. `SiteHeader` is back on `/`, the hero is a
single column of the spec §11 copy plus the two CTAs, and the page returns to
being one committed paper surface end to end.
...
| `components/ui/*` | Primitives, header (wordmark only), demo reset, word-reveal. **No hero graphic component** — the hero is type. |
...
| **The landing hero has no graphic at all** | Abhi's direction, and the end of a run of three: the static constellation SVG, then the rotating particle sphere (`recursive-erosion`), then a Spline galaxy scene from the 21st.dev block. He cut each one in turn and then cut the idea. The hero is now the spec §11 copy, the two CTAs and the demo code — nothing else. **Do not put art back in the hero without asking him.** |
| Every hero-graphic component is deleted, not orphaned | `hero-constellation.tsx`, `recursive-erosion.tsx` and `galaxy-interactive-hero-section.tsx` are gone from the tree, and `@splinetool/react-spline` / `@splinetool/runtime` are uninstalled. Dead components in `components/ui/` are exactly what the next agent picks up and re-imports. Recovery is `git show f14bece` if any of it is ever wanted back. |
